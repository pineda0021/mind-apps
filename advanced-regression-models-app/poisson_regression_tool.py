...
def run():
    ...
    def build_equation(model_result, response_name):
        ...
        equation = f"\\log(E[{response_name}]) = {round(params['Intercept'], 4)}"
    ...
    for term in res.params.index:
        ...
        if term == "Intercept":
            ...
            interpretation = (
                ...
                f"the expected count for **{response_original}** has log-mean **{coef:.4f}** "
                f"and expected mean **{np.exp(coef):.4f}**."
            )
