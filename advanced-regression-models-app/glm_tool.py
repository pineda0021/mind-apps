...
""")
    ...
    for name in model.params.index:
        ...
        if name == "Intercept":
            ...
            if coef >= 0:
                interpretation = (
                    f"For {var_name} = {level}, the estimated mean of {response} is {abs(coef):.4f} units larger "
                    f"than for {var_name} = {ref}, holding all other predictors constant."
                )
            else:
                interpretation = (
                    f"For {var_name} = {level}, the estimated mean of {response} is {abs(coef):.4f} units smaller "
                    f"than for {var_name} = {ref}, holding all other predictors constant."
                )
            ...
            if coef >= 0:
                interpretation = (
                    f"With a one-unit increase in {name}, the estimated mean of {response} increases by "
                    f"{abs(coef):.4f} units, holding all other predictors constant."
                )
            else:
                interpretation = (
                    f"With a one-unit increase in {name}, the estimated mean of {response} decreases by "
                    f"{abs(coef):.4f} units, holding all other predictors constant."
                )
