...
# Sidebar dropdown for test selection
...
# --- PAIRED T-TEST USING DATA ---
    ...
    if uploaded_file is not None:
        ...
        if "Sample1" not in df.columns or "Sample2" not in df.columns:
            ...
            st.latex(r"SE = \frac{s_d}{\sqrt{n}} = {sd_diff:.3f}/\sqrt{{{n}}} = {se:.3f}")
            st.latex(r"t = \frac{\bar{d}}{SE} = {t_stat:.3f}")
...
# --- PAIRED T-TEST USING SUMMARY STATISTICS ---
    ...
    if st.button("Calculate"):
        ...
        st.subheader("Step-by-Step Calculation")
        st.latex(r"SE = \frac{s_d}{\sqrt{n}} = " + f"{se:.3f}")
        st.latex(r"t = \frac{\bar{d}}{SE} = " + f"{t_stat:.3f}")
        st.latex(r"t_{{\alpha/2, n-1}} = " + f"{t_crit:.3f}")
        st.write(f"P-Value = {p_val:.4f}")
        st.write("Decision:", "Reject H₀" if abs(t_stat) > t_crit else "Fail to reject H₀")
...
# --- INDEPENDENT T-TEST USING DATA ---
    ...
    if uploaded_file is not None:
        ...
        if "Sample1" not in df.columns or "Sample2" not in df.columns:
            ...
            st.latex(r"SE = \sqrt{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}} = {se:.3f}")
            st.latex(r"t = \frac{\bar{x}_1 - \bar{x}_2}{SE} = {t_stat:.3f}")
...
# --- F-TEST FOR STANDARD DEVIATION ---
    ...
    if st.button("Calculate"):
        ...
        st.subheader("Step-by-Step Calculation")
        st.latex(r"F = \frac{s_1^2}{s_2^2} = " + f"{F:.3f}")
        st.latex(r"df_1 = " + f"{df1}, df_2 = {df2}")
