...
def run():
    ...
    option = st.radio("", [
        ...
        "4. Inverse left: Find x for given P(X̄ < x)",
        "5. Inverse right: Find x for given P(X̄ > x)",
        "6. Inverse between: Find x1 and x2 for given P(x1 < X̄ < x2)"
    ])
    ...
    if option == "1. Probability to the left of x (P(X̄ < x))":
        ...
        if b < a:
            ...
            return
    elif option == "4. Inverse left: Find x for given P(X̄ < x)":
        p = st.number_input("Enter cumulative probability p (0 < p < 1)", min_value=0.0, max_value=1.0, value=0.5)
    elif option == "5. Inverse right: Find x for given P(X̄ > x)":
        p = st.number_input("Enter cumulative probability p (0 < p < 1)", min_value=0.0, max_value=1.0, value=0.5)
    elif option == "6. Inverse between: Find x1 and x2 for given P(x1 < X̄ < x2)":
    ...
    if st.button("📊 Calculate"):
        ...
        if option == "1. Probability to the left of x (P(X̄ < x))":
            ...
            st.success(f"x1 such that P(X̄ < x1) = {p_lower} is {round_prob(x1)}")
            st.success(f"x2 such that P(X̄ < x2) = {p_upper} is {round_prob(x2)}")
            st.success(f"P({round_prob(x1)} < X̄ < {round_prob(x2)}) = {round_prob(prob_between)}")
