...
def run_hypothesis_tool():
    ...
    decimal_places = int(st.number_input("Decimal places for rounding", min_value=1, max_value=10, value=4, step=1))

    # ---------------- Proportion test (large sample) ----------------
    if test_choice == "Proportion test (large sample)":
        with st.form("large_prop_form"):
    ...
    # ---------------- t-test (raw data) ----------------
        ...
        with st.form("t_raw_form"):
            data = None
            upload = st.file_uploader("Upload CSV/Excel or enter data manually", type=["csv", "xlsx"])
            if upload:
                if upload.name.endswith(".csv"):
                    data = pd.read_csv(upload).iloc[:,0].dropna().astype(float).values
                else:
                    data = pd.read_excel(upload).iloc[:,0].dropna().astype(float).values
            raw = st.text_area("Enter comma-separated values (if not uploading a file)")
            if raw:
    ...
    # ---------------- Chi-squared (summary stats) ----------------
    elif test_choice == "Chi-squared test for std dev (summary stats)":
        with st.form("chi_summary_form"):
            sample_std = st.number_input("Sample standard deviation", format="%.10f")
            sample_size = st.number_input("Sample size", min_value=2, step=1)
    ...
    # ---------------- Chi-squared (raw data) ----------------
    elif test_choice == "Chi-squared test for std dev (raw data)":
        with st.form("chi_raw_form"):
            data = None
            upload = st.file_uploader("Upload CSV/Excel or enter data manually", type=["csv", "xlsx"])
            if upload:
                if upload.name.endswith(".csv"):
