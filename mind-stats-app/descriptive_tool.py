# ==========================================================
# Descriptive Statistics Tool
# Plotly Version
# Created by Professor Edward Pineda-Castro
# MIND: Statistics Visualizer Suite
# ==========================================================

import streamlit as st
import numpy as np
import pandas as pd
from scipy import stats
import re
import plotly.express as px
import plotly.graph_objects as go


# ==========================================================
# Helper Functions
# ==========================================================

def get_summary_stats(data, decimals=2):

    minimum = round(np.min(data), decimals)
    q1 = round(np.percentile(data, 25), decimals)
    median = round(np.median(data), decimals)
    q3 = round(np.percentile(data, 75), decimals)
    maximum = round(np.max(data), decimals)
    iqr = round(q3 - q1, decimals)

    lower_bound = q1 - 1.5 * iqr
    upper_bound = q3 + 1.5 * iqr

    outliers = sorted([
        round(x, decimals)
        for x in data
        if x < lower_bound or x > upper_bound
    ])

    mean = round(np.mean(data), decimals)

    mode_result = stats.mode(
        data,
        keepdims=True
    )

    mode = ", ".join(
        map(
            str,
            np.round(
                mode_result.mode,
                decimals
            )
        )
    )

    range_val = round(
        maximum - minimum,
        decimals
    )

    pop_var = round(
        np.var(data, ddof=0),
        decimals
    )

    pop_std = round(
        np.std(data, ddof=0),
        decimals
    )

    samp_var = round(
        np.var(data, ddof=1),
        decimals
    )

    samp_std = round(
        np.std(data, ddof=1),
        decimals
    )

    return {
        "Minimum": minimum,
        "Q1": q1,
        "Median": median,
        "Q3": q3,
        "Maximum": maximum,
        "IQR": iqr,
        "Lower Bound": round(lower_bound, decimals),
        "Upper Bound": round(upper_bound, decimals),
        "Outliers": outliers if outliers else "None",
        "Mean": mean,
        "Mode": mode,
        "Range": range_val,
        "σ² (Population Variance)": pop_var,
        "σ (Population Std Dev)": pop_std,
        "s² (Sample Variance)": samp_var,
        "s (Sample Std Dev)": samp_std
    }


def parse_intervals(interval_text):

    intervals = []

    pattern = r"\[(\s*-?\d+\.?\d*\s*),(\s*-?\d+\.?\d*\s*)\]"

    matches = re.findall(
        pattern,
        interval_text
    )

    for match in matches:

        try:

            low = float(match[0])
            high = float(match[1])

            if low < high:

                intervals.append(
                    (low, high)
                )

        except:
            continue

    return intervals


def compute_frequency_table(data, intervals):

    freq = [
        np.sum(
            (data >= low)
            &
            (data <= high)
        )
        for low, high in intervals
    ]

    total = np.sum(freq)

    rel_freq = [
        round(
            f / total,
            4
        )
        if total > 0
        else 0
        for f in freq
    ]

    cum_freq = np.cumsum(
        freq
    )

    df = pd.DataFrame({

        "Class Interval": [
            f"[{int(low)},{int(high)}]"
            for low, high in intervals
        ],

        "Frequency":
            freq,

        "Relative Freq":
            rel_freq,

        "Cumulative Freq":
            cum_freq

    })

    return df


# ==========================================================
# Quantitative Analyzer
# ==========================================================

def run_quantitative(df_uploaded=None):

    st.subheader(
        "📊 Quantitative Data Analyzer"
    )

    q_type = st.radio(
        "Select Data Type:",
        [
            "Discrete",
            "Continuous"
        ],
        horizontal=True,
        key="quantitative_type"
    )

    input_mode = st.radio(
        "Data Input Mode:",
        [
            "Upload File",
            "Manual Entry"
        ],
        horizontal=True,
        key="quantitative_input_mode"
    )

    data = None


    # ======================================================
    # GET DATA
    # ======================================================

    if input_mode == "Upload File":

        if df_uploaded is None:

            st.warning(
                "Upload a dataset first."
            )

            return

        numeric_cols = (
            df_uploaded
            .select_dtypes(
                include=[np.number]
            )
            .columns
            .tolist()
        )

        if not numeric_cols:

            st.error(
                "No numeric columns found."
            )

            return

        col = st.selectbox(
            "Select numeric column:",
            numeric_cols,
            key="quantitative_column"
        )

        data = (
            df_uploaded[col]
            .dropna()
            .astype(float)
            .values
        )

        st.success(
            f"Loaded {len(data)} observations."
        )

    else:

        example = (
            "728,730,726,698,"
            "721,722,700,720,"
            "729,678"
        )

        raw_data = st.text_area(
            "Enter comma-separated numeric values:",
            example,
            key="quantitative_manual_data"
        )

        try:

            data = np.array([
                float(x.strip())
                for x
                in raw_data.split(",")
                if x.strip()
            ])

            st.success(
                f"Loaded {len(data)} observations."
            )

        except:

            st.error(
                "Invalid numeric input."
            )

            return


    if len(data) == 0:

        st.warning(
            "Please enter numeric data."
        )

        return


    # ======================================================
    # DISCRETE
    # ======================================================

    if q_type == "Discrete":

        if st.button(
            "👨‍💻 Calculate",
            key="quantitative_discrete_calculate"
        ):

            st.session_state[
                "show_discrete_results"
            ] = True

            st.session_state[
                "discrete_data"
            ] = data


        if st.session_state.get(
            "show_discrete_results",
            False
        ):

            discrete_data = (
                st.session_state[
                    "discrete_data"
                ]
            )

            counts = (
                pd.Series(
                    discrete_data
                )
                .value_counts()
                .sort_index()
            )

            freq_df = pd.DataFrame({

                "Value":
                    counts.index,

                "Frequency":
                    counts.values,

                "Relative Frequency":
                    np.round(
                        counts.values
                        / len(discrete_data),
                        4
                    )

            })

            freq_df[
                "Cumulative Frequency"
            ] = (
                freq_df[
                    "Frequency"
                ]
                .cumsum()
            )

            st.markdown(
                "### Frequency Table"
            )

            st.dataframe(
                freq_df,
                use_container_width=True
            )

            fig = px.histogram(
                x=discrete_data,
                nbins=len(
                    np.unique(
                        discrete_data
                    )
                )
            )

            fig.update_layout(
                title="📊 Discrete Histogram",
                xaxis_title="Values",
                yaxis_title="Frequency"
            )

            st.plotly_chart(
                fig,
                use_container_width=True,
                key="discrete_histogram"
            )

        return


    # ======================================================
    # CONTINUOUS
    # ======================================================

    min_val = np.min(data)
    max_val = np.max(data)
    n = len(data)

    k = int(
        np.ceil(
            1
            + 3.322
            * np.log10(n)
        )
    )

    class_width = np.ceil(
        (max_val - min_val)
        / k
    )

    if class_width == 0:
        class_width = 1

    auto_intervals = []

    start = np.floor(
        min_val
    )

    for i in range(k):

        low = (
            start
            + i * class_width
        )

        high = (
            low
            + class_width
            - 1
        )

        auto_intervals.append(
            (low, high)
        )


    default_intervals_text = ", ".join(

        f"[{int(low)},{int(high)}]"

        for low, high
        in auto_intervals

    )


    # ======================================================
    # CALCULATE BUTTON
    # ======================================================

    if st.button(
        "👨‍💻 Calculate",
        key="quantitative_continuous_calculate"
    ):

        st.session_state[
            "show_continuous_results"
        ] = True

        st.session_state[
            "continuous_data"
        ] = data

        st.session_state[
            "continuous_default_intervals"
        ] = default_intervals_text

        st.session_state[
            "continuous_k"
        ] = k

        st.session_state[
            "continuous_class_width"
        ] = class_width


    # ======================================================
    # DISPLAY CONTINUOUS RESULTS
    # ======================================================

    if st.session_state.get(
        "show_continuous_results",
        False
    ):

        continuous_data = (
            st.session_state[
                "continuous_data"
            ]
        )

        saved_intervals_text = (
            st.session_state[
                "continuous_default_intervals"
            ]
        )

        saved_k = (
            st.session_state[
                "continuous_k"
            ]
        )

        saved_class_width = (
            st.session_state[
                "continuous_class_width"
            ]
        )


        st.info(
            f"Generated {saved_k} class intervals "
            f"(width ≈ {int(saved_class_width)})."
        )


        # ==================================================
        # EDIT CLASS INTERVALS
        # ==================================================

        edit = st.checkbox(
            "Edit class intervals?",
            value=False,
            key="edit_class_intervals"
        )


        if edit:

            interval_text = st.text_area(
                "Class Intervals:",
                value=saved_intervals_text,
                key="continuous_interval_text"
            )

            intervals = parse_intervals(
                interval_text
            )

            if not intervals:

                st.error(
                    "Invalid format. "
                    "Use: [60,69], [70,79]"
                )

                return

        else:

            intervals = parse_intervals(
                saved_intervals_text
            )


        # ==================================================
        # FREQUENCY TABLE
        # ==================================================

        df_freq = compute_frequency_table(
            continuous_data,
            intervals
        )

        st.markdown(
            "### Frequency Table"
        )

        st.dataframe(
            df_freq,
            use_container_width=True
        )


        # ==================================================
        # CHOOSE VISUALIZATION
        # ==================================================

        plot_option = st.radio(
            "Choose visualization:",
            [
                "Histogram",
                "Histogram + Ogive",
                "Boxplot"
            ],
            horizontal=True,
            key="continuous_plot_option"
        )


        # ==================================================
        # HISTOGRAM
        # ==================================================

        if plot_option == "Histogram":

            bin_size = (
                intervals[0][1]
                - intervals[0][0]
                + 1
            )

            fig = go.Figure(

                go.Histogram(

                    x=continuous_data,

                    xbins=dict(

                        start=
                            intervals[0][0],

                        end=
                            intervals[-1][1]
                            + 1,

                        size=
                            bin_size

                    )

                )

            )

            fig.update_layout(
                title="📊 Continuous Histogram",
                xaxis_title="Values",
                yaxis_title="Frequency"
            )

            st.plotly_chart(
                fig,
                use_container_width=True,
                key="continuous_histogram"
            )


        # ==================================================
        # HISTOGRAM + OGIVE
        # ==================================================

        elif plot_option == "Histogram + Ogive":

            bin_size = (
                intervals[0][1]
                - intervals[0][0]
                + 1
            )

            fig = go.Figure()

            fig.add_trace(

                go.Histogram(

                    x=continuous_data,

                    xbins=dict(

                        start=
                            intervals[0][0],

                        end=
                            intervals[-1][1]
                            + 1,

                        size=
                            bin_size

                    ),

                    name=
                        "Frequency"

                )

            )


            upper_bounds = [

                high

                for _, high
                in intervals

            ]


            cum_freq = (

                df_freq[
                    "Cumulative Freq"
                ]
                .values

            )


            fig.add_trace(

                go.Scatter(

                    x=
                        upper_bounds,

                    y=
                        cum_freq,

                    mode=
                        "lines+markers",

                    name=
                        "Cumulative Frequency",

                    yaxis=
                        "y2"

                )

            )


            fig.update_layout(

                title=
                    "📊 Histogram + Ogive",

                xaxis_title=
                    "Values",

                yaxis_title=
                    "Frequency",

                yaxis2=dict(

                    overlaying=
                        "y",

                    side=
                        "right",

                    title=
                        "Cumulative Frequency"

                )

            )


            st.plotly_chart(
                fig,
                use_container_width=True,
                key="continuous_histogram_ogive"
            )


        # ==================================================
        # BOXPLOT
        # ==================================================

        else:

            fig = px.box(
                x=continuous_data,
                orientation="h"
            )

            fig.update_layout(
                title="📦 Boxplot",
                xaxis_title="Values"
            )

            st.plotly_chart(
                fig,
                use_container_width=True,
                key="continuous_boxplot"
            )


# ==========================================================
# QUALITATIVE ANALYZER
# ==========================================================

def run_qualitative(df_uploaded=None):

    st.subheader(
        "🎨 Qualitative (Categorical) Analyzer"
    )

    input_mode = st.radio(
        "Input Mode:",
        [
            "Upload File",
            "Manual Entry"
        ],
        horizontal=True,
        key="qualitative_input_mode"
    )


    # ======================================================
    # GET DATA
    # ======================================================

    if input_mode == "Upload File":

        if df_uploaded is None:

            st.warning(
                "Upload file first."
            )

            return


        text_cols = (

            df_uploaded
            .select_dtypes(
                include="object"
            )
            .columns
            .tolist()

        )


        if not text_cols:

            st.error(
                "No categorical columns found."
            )

            return


        col = st.selectbox(
            "Select column:",
            text_cols,
            key="qualitative_column"
        )


        data = (

            df_uploaded[col]
            .dropna()
            .astype(str)
            .tolist()

        )


    else:

        raw_data = st.text_area(
            "Categories:",
            "Red, Blue, Red, Green, Yellow",
            key="qualitative_manual_data"
        )


        data = [

            x.strip()

            for x
            in raw_data.split(",")

            if x.strip()

        ]


    if len(data) == 0:

        st.warning(
            "Please enter categorical data."
        )

        return


    # ======================================================
    # CALCULATE FREQUENCIES
    # ======================================================

    counts = (
        pd.Series(data)
        .value_counts()
    )


    freq_df = pd.DataFrame({

        "Category":
            counts.index.astype(str),

        "Frequency":
            counts.values,

        "Relative Freq":
            np.round(
                counts.values
                / counts.values.sum(),
                4
            )

    })


    # ======================================================
    # CALCULATE BUTTON
    # ======================================================

    if st.button(
        "👨‍💻 Calculate",
        key="qualitative_calculate"
    ):

        st.session_state[
            "show_qualitative_results"
        ] = True


    # ======================================================
    # DISPLAY RESULTS
    # ======================================================

    if st.session_state.get(
        "show_qualitative_results",
        False
    ):

        st.markdown(
            "### Frequency Table"
        )

        st.dataframe(
            freq_df,
            use_container_width=True
        )


        # ==================================================
        # CHOOSE CHART
        # ==================================================

        chart_type = st.radio(
            "Choose chart:",
            [
                "Bar Chart",
                "Pie Chart"
            ],
            horizontal=True,
            key="qualitative_chart_type"
        )


        # ==================================================
        # BAR CHART
        # ==================================================

        if chart_type == "Bar Chart":

            fig = px.bar(
                freq_df,
                x="Category",
                y="Frequency",
                title="🎨 Bar Chart"
            )

            fig.update_layout(
                xaxis_title="Category",
                yaxis_title="Frequency"
            )

            st.plotly_chart(
                fig,
                use_container_width=True,
                key="qualitative_bar_chart"
            )


        # ==================================================
        # PIE CHART
        # ==================================================

        elif chart_type == "Pie Chart":

            fig = px.pie(
                freq_df,
                names="Category",
                values="Frequency",
                title="🥧 Pie Chart"
            )

            fig.update_traces(
                textposition="inside",
                textinfo="label+percent",
                hovertemplate=(
                    "<b>%{label}</b><br>"
                    "Frequency: %{value}<br>"
                    "Percent: %{percent}"
                    "<extra></extra>"
                )
            )

            fig.update_layout(
                showlegend=True
            )

            st.plotly_chart(
                fig,
                use_container_width=True,
                key="qualitative_pie_chart"
            )


# ==========================================================
# SUMMARY STATS + BOX PLOTS
# ==========================================================

def run_summary(df_uploaded=None):

    st.subheader(
        "📊 Summary Statistics & Boxplots"
    )


    mode = st.radio(
        "Mode:",
        [
            "Single Dataset",
            "Multiple Datasets"
        ],
        horizontal=True,
        key="summary_mode"
    )


    # ======================================================
    # SINGLE DATASET
    # ======================================================

    if mode == "Single Dataset":

        raw = st.text_area(
            "Numbers:",
            "56, 57, 54, 61, 63, 58, 59, 62",
            key="summary_single_data"
        )


        if st.button(
            "👨‍💻 Calculate",
            key="summary_single_calculate"
        ):

            data = np.array([

                float(x.strip())

                for x
                in raw.split(",")

                if x.strip()

            ])


            summary = pd.DataFrame(

                get_summary_stats(
                    data
                ).items(),

                columns=[
                    "Statistic",
                    "Value"
                ]

            )


            st.dataframe(
                summary,
                use_container_width=True
            )


            fig = px.box(
                x=data,
                orientation="h"
            )


            fig.update_layout(
                title="📦 Boxplot",
                xaxis_title="Values"
            )


            st.plotly_chart(
                fig,
                use_container_width=True
            )


    # ======================================================
    # MULTIPLE DATASETS
    # ======================================================

    else:

        raw = st.text_area(
            "Enter datasets separated by semicolons:",
            "56,57,54; 49,51,55; 65,64,68",
            key="summary_multiple_data"
        )


        if st.button(
            "👨‍💻 Calculate",
            key="summary_multiple_calculate"
        ):

            blocks = [

                b.strip()

                for b
                in raw.split(";")

                if b.strip()

            ]


            data_dict = {}


            for i, block in enumerate(
                blocks,
                1
            ):

                data_dict[
                    f"Dataset {i}"
                ] = np.array([

                    float(x.strip())

                    for x
                    in block.split(",")

                    if x.strip()

                ])


            combined = pd.DataFrame()


            for name, d in data_dict.items():

                stats_dict = (
                    get_summary_stats(d)
                )


                df_stats = pd.DataFrame(
                    stats_dict,
                    index=[name]
                )


                combined = pd.concat([
                    combined,
                    df_stats
                ])


            st.dataframe(
                combined,
                use_container_width=True
            )


            fig = go.Figure()


            for name, d in data_dict.items():

                fig.add_trace(

                    go.Box(

                        x=d,

                        name=name,

                        orientation="h"

                    )

                )


            fig.update_layout(
                title="📦 Boxplots (Multiple)",
                xaxis_title="Values"
            )


            st.plotly_chart(
                fig,
                use_container_width=True
            )


# ==========================================================
# MAIN APP
# ==========================================================

def run():

    st.header(
        "✏️ Descriptive Statistics Tool"
    )


    categories = [

        "Qualitative (Categorical)",

        "Quantitative (Discrete or Continuous)",

        "Summary Statistics & Boxplot"

    ]


    choice = st.selectbox(
        "Choose a category:",
        categories,
        index=None,
        key="main_category"
    )


    if not choice:

        st.info(
            "👆 Please select a category to begin."
        )


    uploaded_file = st.file_uploader(
        "📂 Upload CSV or Excel (optional):",
        type=[
            "csv",
            "xlsx"
        ]
    )


    df_uploaded = None


    if uploaded_file:

        try:

            df_uploaded = (

                pd.read_csv(
                    uploaded_file
                )

                if uploaded_file.name.endswith(
                    ".csv"
                )

                else pd.read_excel(
                    uploaded_file
                )

            )


            st.success(
                "File uploaded successfully!"
            )


        except Exception as e:

            st.error(
                f"Error loading file: {e}"
            )

            return


    if choice == "Qualitative (Categorical)":

        run_qualitative(
            df_uploaded
        )


    elif choice == "Quantitative (Discrete or Continuous)":

        run_quantitative(
            df_uploaded
        )


    elif choice == "Summary Statistics & Boxplot":

        run_summary(
            df_uploaded
        )


if __name__ == "__main__":

    run()
         



