        ...
        if st.button("👨‍💻 Calculate"):
            ...
            # ---------- MIDDLE
                ...
                st.latex(rf"""
                    ...
                    x_1 = {x_lower:.{decimal}f},\quad
                    x_2 = {x_upper:.{decimal}f}
                """)
...
def sampling_mean(decimal):
    ...
    st.write(
        f"**Standard Error (σx̄) = {round(se, decimal)}**"
    )
    ...
    # ------- P(a < X̄ < b)
        ...
        if st.button("👨‍💻 Calculate"):
            ...
            st.latex(rf"""
                Z_1 = {z1:.{decimal}f},\quad
                Z_2 = {z2:.{decimal}f} \\[6pt]
                P({z1:.{decimal}f} < Z < {z2:.{decimal}f})
                = {prob:.{decimal}f} \\[6pt]
...
def sampling_proportion(decimal):
    ...
    st.write(
        f"**Standard Error (σp̂) = {round(se, decimal)}**"
    )
...
def uniform_distribution(decimal):
    ...
    if b <= a:
        st.error(
            "⚠️ Upper bound (b) must be greater than lower bound (a)."
        )
        return
    ...
    st.write(
        f"**Constant PDF:** f(x) = {round(pdf, decimal)}"
    )
