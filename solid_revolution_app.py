...
# --- Helper function to parse user expressions ---
...
def plot_solid(top_expr, bottom_expr, method, axis):
    ...
    ax.set_title(f'Greatest Professor\'s {method} about the {axis}')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    ax.view_init(30, 45)
...
# --- Streamlit UI ---
st.title("🧠 Solids of Revolution - Greatest Professor's Visualizer")

top_expr = st.text_input("Top Function (in terms of x or y):", value="x")
bottom_expr = st.text_input("Bottom Function (in terms of x or y):", value="x**2")
method = st.selectbox("Method:", ["Disk/Washer", "Cylindrical Shell"])
axis = st.selectbox("Axis of Rotation:", ["x-axis", "y-axis"])

if st.button("Plot Solid"):
    try:
