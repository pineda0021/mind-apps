...
def step_by_step_derivation(expr):
    ...
    # Case: constant
    if expr.is_Number:
        steps.append(rf"Constant Rule: $\frac{{d}}{{dx}}[{sp.latex(expr)}] = 0$")
        return steps
    ...
    if expr.is_Add:
        ...
        for term in terms:
            steps += step_by_step_derivation(term)
        return steps
    ...
    if expr.is_Pow:
        ...
        if base.has(x):
            ...
            steps.append(rf"Power Rule (constant base): $\frac{{d}}{{dx}}[{sp.latex(expr)}] = 0$")
        return steps

    # Case: sin, cos, exp, log, etc.
    if expr.func in [sp.sin, sp.cos, sp.tan, sp.exp, sp.log]:
        steps.append(rf"Rule for {expr.func.__name__}: $\frac{{d}}{{dx}}[{sp.latex(expr)}] = {sp.latex(sp.diff(expr, x))}$")
        return steps

    # Fallback: general derivative
